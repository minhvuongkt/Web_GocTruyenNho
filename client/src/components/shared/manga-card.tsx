...
export function MangaCard({ manga, genres, featured = false }: MangaCardProps) {
  ...
  return (
    <Link href={`/truyen/${manga.id}`}>
      <Card
        className={`bg-white dark:bg-slate-900 overflow-hidden hover:shadow-lg transition-all ${featured ? "hover:scale-105" : "hover:scale-102"}`}
      >
        ...
        <CardContent className={`p-3 ${featured ? "" : "p-2"}`}>
          <h3
            className={`font-medium text-foreground truncate ${featured ? "text-base" : "text-sm"}`}
          >
            {manga.title}
          </h3>
            ...
            <div className="flex flex-wrap gap-1 mt-2">
              {genres.slice(0, 2).map((genre) => (
                <Badge
                  key={genre.id}
                  variant="outline"
                  className="text-xs px-1.5 py-0 h-5 bg-primary/5 text-primary"
                >
                  {genre.name}
                </Badge>
              ))}
