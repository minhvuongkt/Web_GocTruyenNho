...
// Icons
import { AlertTriangle, ArrowLeft, Bold, Italic, Underline, Upload, File, Image, X, UploadCloud, FileUp } from "lucide-react";
...
export default function ChapterNewPage({ contentId }: { contentId: number }) {
  ...
  const [referenceChapterId, setReferenceChapterId] = useState<number | null>(null);
  const [insertRelative, setInsertRelative] = useState<"before" | "after">("after");
    ...
    onSuccess: (data) => {
      ...
      if (data.imageUrls && data.imageUrls.length > 0) {
        const imageContent = data.imageUrls
          .map((url: string) => `<img src="${url}" alt="Trang truyện" class="manga-page" />`)
          .join('\n');
  ...
  const processZipMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        ...
        description: error instanceof Error ? error.message : "Không thể xử lý file ZIP",
        variant: "destructive",
      });
    }
  });
  ...
  const processTextFileMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        ...
        description: error instanceof Error ? error.message : "Không thể đọc file văn bản",
        variant: "destructive",
      });
    }
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (content?.type === 'manga') {
      ...
      chapterImages.forEach((file, index) => {
        ...
        formData.append('imageNames', `image_${index + 1}_${Date.now()}.${file.name.split('.').pop()}`);
      });
    ...
    <AdminLayout>
      <div className="container mx-auto py-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div>
            <Button variant="outline" onClick={() => navigate(`/admin/chapters/${contentId}`)}>
              <ArrowLeft className="mr-2 h-4 w-4" />
        ...
        <form onSubmit={handleSubmit}>
              ...
              <div className="grid gap-6">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <RadioGroup value={insertPosition} onValueChange={setInsertPosition} className="flex flex-col space-y-1">
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="end" id="option-end" />
                        <Label htmlFor="option-end">Tự động chèn vào cuối</Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="specific" id="option-specific" />
                        <Label htmlFor="option-specific">Chèn vào vị trí cụ thể</Label>
                      </div>
                    </RadioGroup>
                    ...
                    {insertPosition === 'specific' && (
                      <div className="mt-4 grid grid-cols-2 gap-4">
                        <div>
                          <Label htmlFor="referenceChapter">Chương tham chiếu</Label>
                          <Select 
                            value={referenceChapterId?.toString() || ""} 
                            onValueChange={(value) => setReferenceChapterId(parseInt(value))}
                          >
                            <SelectTrigger>
                              ...
                              {chaptersLoading ? (
                                ...
                                chapters.map((chapter: any) => (
                                  <SelectItem key={chapter.id} value={chapter.id.toString()}>
                                    {chapter.number}. {chapter.title || `Chương ${chapter.number}`}
                                  </SelectItem>
                                ))
                              )}
                            </SelectContent>
                        ...
                        <div>
                          ...
                          <Select 
                            ...
                            onValueChange={(value) => setInsertRelative(value as "before" | "after")}
                ...
                <div className="space-y-4">
                  <Tabs defaultValue="image-upload">
                    <TabsList className="mb-4">
                      <TabsTrigger value="image-upload">Tải lên ảnh</TabsTrigger>
                      <TabsTrigger value="zip-upload">Tải lên file ZIP</TabsTrigger>
                    </TabsList>
                    ...
                    <TabsContent value="image-upload">
                      ...
                      >
                        <div className="flex flex-col items-center justify-center space-y-2">
                          ...
                          <h3 className="font-medium">Kéo thả hoặc nhấn để tải lên ảnh</h3>
                      ...
                      {chapterImages.length > 0 && (
                        <div className="mt-6">
                          <h3 className="font-medium mb-2">Ảnh đã tải lên ({chapterImages.length})</h3>
                    ...
                    <TabsContent value="zip-upload">
                      ...
                      >
                        <div className="flex flex-col items-center justify-center space-y-2">
                          ...
                          <h3 className="font-medium">Nhấn để tải lên file ZIP</h3>
                          <p className="text-sm text-muted-foreground">
                            Tải lên file ZIP chứa các ảnh đã được đánh số thứ tự.<br />
          ...
          <div className="mt-6 flex justify-end gap-4">
            ...
            >
              {(createChapterMutation.isPending || uploadImagesMutation.isPending || processZipMutation.isPending) ? (
                <>
