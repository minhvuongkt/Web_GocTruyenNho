...
export function PaymentCallbackPage() {
  ...
  useEffect(() => {
    ...
    const cancelled =
      params.get("cancel") === "true" || params.get("cancel") === "false";
    ...
    const processPayment = async () => {
      try {
        ...
        if (paymentStatus === "PAID" || code === "00") {
          ...
          if (orderCodeParam) {
            const response = await apiRequest(
              "GET",
              `/api/payos/status/${orderCodeParam}`,
            );
            if (response.ok) {
              ...
              if (paymentData.status === "PAID") {
                ...
                setMessage(
                  "Thanh toán thành công! Số tiền đã được nạp vào tài khoản của bạn.",
                );
                ...
                toast({
                  ...
                  description:
                    "Giao dịch đã được hoàn tất và cập nhật tài khoản của bạn.",
                  variant: "default",
                });
        ...
        setStatus("error");
        setMessage(
          "Có lỗi xảy ra trong quá trình xử lý thanh toán. Vui lòng kiểm tra lại sau.",
        );
      } catch (error) {
