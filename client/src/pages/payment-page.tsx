...
export function PaymentPage() {
  ...
  const calculateBonus = (
    amount: number,
  ): { bonus: number; percentage: number } => {
    if (
      !paymentSettings?.discountTiers ||
      !paymentSettings.discountTiers.length
    ) {
      return { bonus: 0, percentage: 0 };
    }

    // Sort tiers by amount in descending order to get the highest applicable tier
    const sortedTiers = [...paymentSettings.discountTiers].sort(
      (a, b) => b.amount - a.amount,
    );

    // Find the applicable tier
    const applicableTier = sortedTiers.find(
      (tier: { amount: number; discountPercent: number }) =>
        amount >= tier.amount,
    );
    ...
    const bonusAmount = Math.floor(
      (amount * applicableTier.discountPercent) / 100,
    );
  ...
  const createPaymentMutation = useMutation({
    mutationFn: async () => {
      ...
      if (amountValue < minimumDeposit) {
        throw new Error(
          `Số tiền tối thiểu là ${formatCurrency(minimumDeposit)} VNĐ`,
        );
      }
    ...
    onSuccess: (data) => {
      if (data.success) {
        ...
        toast({
          ...
          description:
            data.message || "Chưa nhận được tiền, vui lòng thử lại sau",
          variant: "destructive",
        });
      }
    },
          ...
          apiRequest("POST", "/api/payments/update-status", {
            ...
            .then(() => {
              ...
              toast({
                ...
                description:
                  "Giao dịch đã được đánh dấu là thất bại do hết thời gian. Vui lòng tạo giao dịch mới.",
                variant: "destructive",
              });
        ...
        <div className="max-w-3xl mx-auto">
          <Card>
                    ...
                    <div className="max-w-md mx-auto space-y-6">
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-2 pt-2">
                          {paymentSettings?.discountTiers?.map(
                            (tier: {
                              amount: number;
                              ...
                              <Button
                                ...
                                type="button"
                                variant={
                                  presetAmount === tier.amount.toString()
                                    ? "default"
                                    : "outline"
                                }
                                size="sm"
                        ...
                        <p className="text-xs text-muted-foreground">
                          Tối thiểu {formatCurrency(minimumDeposit)} VNĐ. Số
                          tiền phải là bội số của 1,000 VNĐ.
                        </p>

                        {/* Show bonus calculation */}
                        {amount &&
                          !isNaN(parseInt(amount)) &&
                          parseInt(amount) >= minimumDeposit && (
                            <div className="rounded-md bg-muted/50 p-3 mt-2">
                              <div className="flex justify-between text-sm">
                                <span>Nạp:</span>
                                <span>
                                  {formatCurrency(parseInt(amount))} VNĐ
                                </span>
                              </div>
                              ...
                              {calculateBonus(parseInt(amount)).bonus > 0 && (
                                <div className="flex justify-between text-sm text-primary">
                                  <span>
                                    Thưởng (
                                    {
                                      calculateBonus(parseInt(amount))
                                        .percentage
                                    }
                                    %):
                                  </span>
                                  <span>
                                    +
                                    {formatCurrency(
                                      calculateBonus(parseInt(amount)).bonus,
                                    )}{" "}
                                    VNĐ
                                  </span>
                                </div>
                      ...
                      <div className="space-y-4">
                        ...
                        {paymentMethod === "bank_transfer" && (
                          <div className="rounded-md bg-muted p-3 text-xs text-muted-foreground">
                            <p>
                              Chuyển khoản thông qua ứng dụng ngân hàng của bạn.
                              Hệ thống sẽ tự động xác nhận giao dịch trong vòng
                              vài phút.
                            </p>
                          </div>
                        )}
                        ...
                        {paymentMethod === "payos" && (
                          <div className="rounded-md bg-muted p-3 text-xs text-muted-foreground">
                            <p>
                              Thanh toán an toàn qua cổng thanh toán PayOS, hỗ
                              trợ thẻ ngân hàng nội địa, thẻ quốc tế và ví điện
                              tử.
                            </p>
                          </div>
                        )}
                        ...
                        paymentMethod === "bank_transfer" && (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="rounded-md bg-muted p-3 text-xs text-muted-foreground">
                                <p>
                                  Sau khi chuyển khoản, vui lòng đợi hệ thống
                                  xác nhận (thường trong vòng 5 phút). Tiền sẽ
                                  được cộng vào tài khoản sau khi xác nhận thành
                                  công.
                                </p>
                              </div>
                              ...
                              {paymentDate && (
                                <div className="flex flex-col gap-2 pt-2 pb-2">
                                  ...
                                  <p className="text-xs text-muted-foreground">
                                    Giao dịch sẽ tự động hết hạn sau 10 phút nếu
                                    không được xác nhận
                                  </p>
                                </div>
                              )}
                              ...
                              <PayOSDirectCheckout
                                ...
                                onSuccess={(transId) => {
                                  toast({
                                    ...
                                    description:
                                      "Tiền đã được nạp vào tài khoản của bạn",
                                  });
            ...
            <CardFooter className="flex justify-between">
              <p className="text-xs text-muted-foreground">
                Nếu gặp vấn đề trong quá trình thanh toán, vui lòng liên hệ hỗ
                trợ.
              </p>
            </CardFooter>
          </Card>
