...
export function generateQRCode(params: QRCodeParams): string {
  ...
  const payload = [
    "00",
    "02",
    "01", // Payload format indicator and version
    "01",
    "12",
    "VRPTMRPTPAYEE", // Merchant account information tag
    "38",
    "09",
    bankBin, // Merchant bin (default: MB Bank)
    "01",
    bankNumber.length.toString().padStart(2, "0"),
    bankNumber, // Merchant ID
    "52",
    "04",
    "QRCT", // Banking category
  ];
  ...
  if (amount > 0) {
    payload.push(
      "54",
      formatAmount.length.toString().padStart(2, "0"),
      formatAmount,
    );
  }
