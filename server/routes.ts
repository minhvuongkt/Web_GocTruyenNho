...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/content/search/advanced', async (req, res) => {
    try {
      ...
      if (req.query.status) {
        searchParams.status = req.query.status as 'ongoing' | 'completed' | 'hiatus';
      }
      
      if (req.query.genres) {
        const genreIds = (req.query.genres as string).split(',').map(id => parseInt(id.trim()));
      ...
      const result = await storage.searchContentAdvanced(searchParams, page, limit);
      res.json(result);
    } catch (error) {
      res.status(500).json({ error: 'Failed to search content' });
    }
  ...
  app.patch('/api/content/:id', ensureAdmin, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const contentData = req.body;
      const genreIds = contentData.genreIds;
      ...
      const updatedContent = await storage.updateContent(id, contentData, genreIds);
      if (!updatedContent) {
        return res.status(404).json({ error: 'Content not found' });
      }
  ...
  app.post('/api/content/:contentId/chapters', ensureAdmin, async (req, res) => {
    try {
      const contentId = parseInt(req.params.contentId);
      const chapterData = {
        ...req.body,
  ...
  app.post('/api/chapters/:chapterId/content', ensureAdmin, async (req, res) => {
    try {
  ...
  app.post('/api/chapters/:id/unlock', ensureAuthenticated, async (req, res) => {
    try {
      ...
      if (!chapter.unlockPrice) {
        return res.status(400).json({ error: 'Chapter does not have an unlock price' });
      }
      ...
      const alreadyUnlocked = await storage.isChapterUnlocked(userId, chapterId);
      if (alreadyUnlocked) {
        return res.status(400).json({ error: 'Chapter already unlocked by this user' });
      }
      
      // Unlock the chapter and deduct balance
      await storage.unlockChapter(userId, chapterId);
      await storage.updateUserBalance(userId, user.balance - chapter.unlockPrice);
  ...
  app.post('/api/payments', ensureAuthenticated, async (req, res) => {
    try {
      ...
      const transactionId = createHash('sha256').update(`${userId}-${Date.now()}`).digest('hex').substring(0, 12);
  ...
  app.post('/api/payments/:id/confirm', ensureAuthenticated, async (req, res) => {
    try {
      ...
      const expiryTime = new Date(payment.createdAt.getTime() + 10 * 60 * 1000);
      ...
      const updatedPayment = await storage.updatePaymentStatus(paymentId, 'pending');
  ...
  app.get('/api/ads', async (req, res) => {
    try {
      ...
      if (req.query.all === 'true' && req.isAuthenticated() && (req.user as any).role === 'admin') {
        const page = parseInt(req.query.page as string) || 1;
        const limit = parseInt(req.query.limit as string) || 10;
  ...
  app.post('/api/ads', ensureAdmin, upload.single('image'), async (req, res) => {
    try {
  ...
  app.patch('/api/ads/:id', ensureAdmin, upload.single('image'), async (req, res) => {
    try {
      const id = parseInt(req.params.id);
